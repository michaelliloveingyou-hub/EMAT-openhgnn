...
class AutoML(object):
    ...
    def _objective(self, trials):
        ...
        metrics = flow.train()['metric']
        # Use validation metrics for hyper-parameter selection. Falling back to test is only for legacy flows
        # that do not report a validation metric.
        result = metrics.get('valid', metrics.get('test'))
        if result is None:
            raise KeyError("HPO requires a 'valid' or 'test' metric in flow.train()['metric'].")
