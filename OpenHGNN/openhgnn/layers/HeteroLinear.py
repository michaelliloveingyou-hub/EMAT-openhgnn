...
class HeteroFeature(nn.Module):
    r"""
    This is a feature preprocessing component for heterogeneous node features.

    ASHIN-D/F/G/H 的目标节点特征会被拆成 raw 和 ASHIN 两段，在这里各自映射后再学一个 gate。
    B/C/E，或者没有写进 ashin_split_dims 的节点类型，继续走 OpenHGNN 原来的 HeteroLinear 逻辑。
    """
    ...
    def __init__(
        ...
        self.act = act
        self.ashin_fusion = ashin_fusion
        self.ashin_split_dims = ashin_split_dims or {}
        # builder.py 只会给需要门控的目标节点写 split dims。
        # 这里用 gated_types 做分流，避免影响其他节点类型的原始特征预处理。
        self.gated_types = set(self.ashin_split_dims.keys()) if ashin_fusion == "gated" else set()
        ...
        linear_dict = {}
        embed_dict = {}
        self.raw_linear = nn.ModuleDict()
        self.ashin_linear = nn.ModuleDict()
        self.gate_linear = nn.ModuleDict()
        for ntype, n_nodes in self.n_nodes_dict.items():
            h = h_dict.get(ntype)
            if h is None:
                if all_feats:
                    embed_dict[ntype] = n_nodes
            elif ntype in self.gated_types:
                # 拼接发生在图特征写回阶段；到这里需要按记录的维度切开。
                # 维度先严格检查，防止 cache 或 best_config 混用后悄悄训练出错误结果。
                dims = self.ashin_split_dims[ntype]
                raw_dim = int(dims["raw_dim"])
                ashin_dim = int(dims["ashin_dim"])
                expected_dim = raw_dim + ashin_dim
                if h.shape[1] != expected_dim:
                    raise ValueError(
                        f"ASHIN gated feature dim mismatch for {ntype}: got {h.shape[1]}, expected {expected_dim}."
                    )
                self.raw_linear[ntype] = nn.Linear(raw_dim, embed_size, bias=True)
                self.ashin_linear[ntype] = nn.Linear(ashin_dim, embed_size, bias=True)
                self.gate_linear[ntype] = nn.Linear(expected_dim, embed_size, bias=True)
            else:
                linear_dict[ntype] = h.shape[1]
        self.embes = HeteroEmbedding(embed_dict, embed_size)
        self.linear = HeteroLinear(linear_dict, embed_size) if need_trans and linear_dict else None

    def _gated_forward_one(self, ntype, h):
        dims = self.ashin_split_dims[ntype]
        raw_dim = int(dims["raw_dim"])
        raw_h = h[:, :raw_dim]
        ashin_h = h[:, raw_dim:raw_dim + int(dims["ashin_dim"])]
        raw_z = self.raw_linear[ntype](raw_h)
        ashin_z = self.ashin_linear[ntype](ashin_h)
        # gate 是逐节点、逐隐藏维度的权重；不是一个全局标量。
        # 这样模型可以在不同维度上自己决定 ASHIN 特征该加多少。
        gate = torch.sigmoid(self.gate_linear[ntype](h))
        out = raw_z + gate * ashin_z
        if self.act:
            out = self.act(out)
        return F.normalize(out, p=2, dim=1)

    def _linear_forward(self, h_dict):
        out = {}
        normal_h = {k: v for k, v in h_dict.items() if k not in self.gated_types}
        if self.linear is not None and normal_h:
            tmp = self.linear(normal_h)
            if self.act:
                for x, y in tmp.items():
                    tmp.update({x: self.act(y)})
            out.update(tmp)
        for ntype, h in h_dict.items():
            if ntype in self.gated_types:
                out[ntype] = self._gated_forward_one(ntype, h)
        return out

    def forward(self):
        out_dict = {}
        out_dict.update(self.embes.weight)
        out_dict.update(self._linear_forward(self.h_dict))
        return out_dict
    ...
    def forward_nodes(self, id_dict):
        ...
        tmp = self._linear_forward(h_dict)
