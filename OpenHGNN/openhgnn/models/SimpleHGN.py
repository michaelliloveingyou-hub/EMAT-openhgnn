...
功能简介：
OpenHGNN 中 SimpleHGN 模型的实现文件。

ASHIN 修改说明：
ASHIN 本身没有改动 SimpleHGN 的消息传递逻辑。
本文件只增加了一个兼容性分支：当 Optuna 搜索到 num_layers=1 时，
SimpleHGN 可以直接构造单层输出层，避免原始多层写法在 num_layers=1 时
出现隐藏维度和类别维度不一致的问题。

严格性说明：
- num_layers > 1 时，SimpleHGN 仍走原始多层构造逻辑。
- 该修改不读取 label / mask / split，不影响数据划分。
- 该修改只保证调参搜索空间中的 num_layers=1 可以正常训练。
"""
...
class SimpleHGN(BaseModel):
    ...
    def __init__(self, edge_dim, num_etypes, in_dim, hidden_dim, num_classes,
        ...
        if num_layers <= 1:
            # Optuna 搜索空间允许 num_layers=1。
            # 原始 SimpleHGN 默认至少包含输入层和输出层；
            # 这里显式构造“单层直接输出类别”的情况。
