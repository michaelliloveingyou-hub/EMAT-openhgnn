...
各类 trainerflow 都会经过 BaseFlow。ASHIN 只在这里接入一次：
task 构建出 DGLHeteroGraph 后，如果 args.use_ashin=True，就调用
apply_ashin_features() 把 B/C/D/E/F/G/H 的结构特征写回图。

concat 版本直接改节点特征；D/F/G/H 还会把 raw/ASHIN 的切分维度交给
HeteroFeature，后面由 HeteroFeature 做门控融合。
...
from ..ashin import apply_ashin_features
...
class BaseFlow(ABC):
    ...
    def __init__(self, args):
        ...
        hg = self.task.get_graph()
        if getattr(self.args, 'use_ashin', False):
            # ASHIN 的唯一训练流程接入点。
            # 这里先改图上的输入特征，再把图交给原始 OpenHGNN 后续流程。
            # 后续模型不需要知道 ASHIN 是怎么构造的，只看到已经增强过的 h/feat。
            hg = apply_ashin_features(hg, self.task.dataset, self.args)
            self.task.dataset.g = hg
    ...
    def feature_preprocess(self, act):
        ...
        if self.hg.ndata.get('h', {}) == {} or self.args.feat == 2:
            ...
            self.input_feature = HeteroFeature({}, get_nodes_dict(self.hg), self.args.hidden_dim,
                                            act=act, **self._ashin_feature_kwargs()).to(self.device)
    ...
    def init_feature(self, act):
        ...
        if isinstance(self.hg.ndata['h'], dict):
            ...
            input_feature = HeteroFeature(self.hg.ndata['h'], get_nodes_dict(self.hg),
                                            self.args.hidden_dim, act=act, **self._ashin_feature_kwargs()).to(self.device)
    ...
    def _ashin_feature_kwargs(self):
        return {
            "ashin_split_dims": getattr(self.args, "ashin_split_dims", None),
            "ashin_fusion": getattr(self.args, "ashin_fusion", "concat"),
        }
