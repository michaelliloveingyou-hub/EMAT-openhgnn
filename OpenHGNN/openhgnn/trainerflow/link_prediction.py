        ...
        if self.args.graphbolt and self.args.mini_batch_flag:
            ...
            if self.args.test_flag:
                if self.args.dataset in ['HGBl-amazon', 'HGBl-LastFM', 'HGBl-PubMed']:
                    ...
                    valid_score = self._test_step(split="valid")
                    test_score = self._test_step(split="test")
                    metric_score = {}
                    metric_score.update(valid_score)
                    metric_score.update(test_score)
                    self.logger.train_info(self.logger.metric2str(metric_score))
                    return dict(metric=metric_score, epoch=epoch)
