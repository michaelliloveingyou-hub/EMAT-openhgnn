...
DEFAULT_MODELS = ["HGT", "SimpleHGN", "GCN", "GAT"]
DEFAULT_FEATURE_MODES = ["A", "B", "C", "D", "E"]
DEFAULT_OUTPUT_ROOT = PROJECT_ROOT / "experiments" / "lisan_model_feature_sweep"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run HPO and multi-seed evaluation for selected Lisan models across "
            "feature modes, datasets, and tasks."
        )
    )
    parser.add_argument("--models", nargs="+", default=DEFAULT_MODELS, choices=DEFAULT_MODELS)
    parser.add_argument("--feature_modes", nargs="+", default=DEFAULT_FEATURE_MODES, choices=FEATURE_MODES)
    ...
    parser.add_argument("--n_trials", type=int, default=50, help="Optuna trials per combination.")
...
def main() -> int:
    ...
    print("Planned Lisan model-feature sweep combinations:")
    runnable = 0
    skipped = 0
    for spec in specs:
        ...
        print(f"{spec.dataset}\t{spec.task}\t{spec.model}\t{spec.feature_mode}\t{status}")
    print(f"Planned total={len(specs)} run={runnable} skip={skipped}")
    ...
    for index, spec in enumerate(specs, start=1):
        print(
            ...
            f"{spec.model} / {spec.feature_mode}"
        )
        row = run_combination(
            spec=spec,
        ...
        print(
            ...
            f"{spec.model} / {spec.feature_mode}: {row.get('status')}"
        )
